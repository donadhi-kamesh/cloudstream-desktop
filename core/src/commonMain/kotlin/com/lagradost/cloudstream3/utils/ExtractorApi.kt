...
suspend fun loadExtractor(
    ...
    Log.d("Extractor", "loadExtractor url=$currentUrl referer=$referer extractors=${extractorApis.size}")
    ...
    for (index in extractorApis.lastIndex downTo 0) {
        ...
        if (compareUrl.startsWith(extractor.mainUrl.replace(schemaStripRegex, ""))) {
            Log.i("Extractor", "prefix-match ${extractor.name} ${extractor.mainUrl}")
            try {
    ...
    for (index in extractorApis.lastIndex downTo 0) {
        ...
        if (Levenshtein.partialRatio(
            ...
            Log.i("Extractor", "fuzzy-match ${extractor.name} ${extractor.mainUrl}")
            try {
                extractor.getUrl(currentUrl, referer, subtitleCallback, callback)
            } catch (e: Exception) {
                logError(e)
    ...
    val host = runCatching { Url(currentUrl).host.lowercase() }.getOrNull()
    if (host != null) {
        val bare = host.removePrefix("www.")
        for (index in extractorApis.lastIndex downTo 0) {
            val extractor = extractorApis[index]
            val eHost = runCatching { Url(extractor.mainUrl).host.lowercase() }.getOrNull() ?: continue
            val eBare = eHost.removePrefix("www.")
            if (bare == eBare || host.endsWith(".$eBare") || eHost.endsWith(".$bare")) {
                Log.i("Extractor", "host-match ${extractor.name} $eHost")
                try {
                    extractor.getUrl(currentUrl, referer, subtitleCallback, callback)
                } catch (e: Exception) {
                    logError(e)
    ...
    Log.w("Extractor", "no extractor matched $currentUrl")
    return false
}
